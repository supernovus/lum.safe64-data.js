...
/**
 * Data64 Core Transcoder
 * 
 * This is the transcoder base class with a minimal
 * profile that only supports the `JSON` format.
 * 
 * @exports module:@lumjs/safe64-data/transcoder
 */
class Data64Core
{
  /**
  ...
  /**
   * A getter that returns the supported formats.
   * Override in sub-classes to create custom profiles.
   */
  get formats()
  {
    return MIN_FORMATS;
  ...
  getFormat(fmt=this.options.format)
  {
    ...
    { // A plugin loader found.
      ...
      if (plugin === false)
      {
        throw new RangeError("Format is not enabled: "+fmt);
      }
      if (typeof plugin === F)
    ...
    {
      throw new TypeError("Invalid format id: "+fmt);
    }
  }
  ...
  /**
   * Create a sub-class of the transcoder core that
   * supports a certain subset of formats.
   * 
   * @param {object} FORMATS - Supported formats;
   * schemata needs to be documented.
   * @returns {class} A sub-class of the Data64 transcoder
   */
  static createProfile(FORMATS)
  {
    const Data64Profile = class extends Data64Core
    {
      get formats() { return FORMATS; }
    }
    return Object.assign(Data64Profile, {FORMATS});
  }

} // Data64 class

// Add some static class properties.
Object.assign(Data64Core, C, 
{
  FORMATS: MIN_FORMATS, Header, Settings,
});

module.exports = Data64Core;
